...
- [fivedrisk](https://github.com/theDoc001/fivedrisk): Runtime governance for LangGraph pipelines. Ships `fivedrisk_gate_node` that drops into any StateGraph before tool-executing nodes. Scores every action on 5 risk dimensions (Data, Tool, Reversibility, External, Autonomy), routes via conditional edges to HITL on ORANGE or deny on RED. Markov SafetyDrift across action sequences, append-only audit log, NDJSON event stream for SIEM. Apache-2.0. ![GitHub Repo stars](https://img.shields.io/github/stars/theDoc001/fivedrisk?style=social)
